...
以下规则在**每次执行前**和**出现偏离时**必须逐条对照。违反任何一条都会导致任务失败或产生不可预期的行为。

### 环境前置（缺一不可）
- [ ] Node.js 22+ 已安装
- [ ] Chrome 远程调试已启用（`chrome://inspect/#remote-debugging` 勾选）
- [ ] CDP Proxy 在 `localhost:3456` 运行（`check-deps.mjs` 自动启动）
- [ ] 已向用户展示风险提示（"存在账号封禁风险"）

### 工具选择（选错 = 拿不到数据）
- [ ] 登录态/反爬平台/交互操作 → **必须 CDP**，不可降级为 WebSearch/WebFetch/curl
- [ ] 搜索引擎/聚合平台只用于**发现**，不可直接用于**证明真伪**
- [ ] 一手来源优先于二手：政策→机构官网，企业→官方新闻页，工具→官方文档/源码

### CDP 操作（不可违背的技术事实）
- [ ] 不操作用户已有 tab，操作在自建后台 tab 中进行
- [ ] 任务结束关闭自建 tab，保留用户 tab
- [ ] **eval 前必须先做最小探针**验证传参（`Content-Type: text/plain` + `--data-raw`）
- [ ] 探针失败 → 检查编码/转义/Content-Type，**不先怀疑页面结构**
- [ ] 点击优先级：① `/click` → ② `/clickAt`（真实鼠标）→ ③ `/setFiles`（文件上传）
- [ ] 提取 URL 保留完整参数，不裁剪
- [ ] 站点内交互产生的链接是可靠的；手动构造的 URL 可能缺失隐式参数
- [ ] 平台"内容不存在"提示不可信，可能是访问方式问题
- [ ] `/scroll` 到底部触发懒加载，提取图片前需先滚动

### 浏览哲学（目标驱动）
- [ ] 先定义成功标准，再行动；每步结果对照成功标准
- [ ] 方向错误立即调整，不在同一方式反复重试
- [ ] 弹窗/登录墙判断：挡住目标→处理，没挡住→绕过

### 并行分治
- [ ] 仅独立目标可分治，子 Agent prompt 必须包含"加载 web-access skill"
- [ ] 子 Agent prompt 描述目标（获取/调研），避免暗示手段（搜索/抓取）

### 完成后
- [ ] 关闭所有自建 tab（`/close`）
- [ ] **不停止 Proxy**（重启需 Chrome 重新授权）
- [ ] 有必要的站点经验主动写入 `references/site-patterns/`
